...
function edit_sort(hod, sort) {
	if (hod.innerHTML == "编辑") {
		...
		// 提交修改请求
		var url = "/Blog/AdminDataServlet?op=sort_update" + "&&old_sort="
				+ temp + "&&new_sort=" + input.value;
		sendURL(url);
	}
}

/**
 * hod 点击的span参数 tag 标签名字
 * 
 * @param hod
 * @param sort
 */
...
function edit_tag(hod, tag) {
	if (hod.innerHTML == "编辑") {
		...
		// 提交修改请求
		var url = "/Blog/AdminDataServlet?op=tag_update" + "&&old_tag="
				+ temp_t + "&&new_tag=" + input_t.value;
		sendURL(url);
	}
}
